...
# -------------------------------------------------------------------
# /admin/orders/<order_id>/status  — admin manual order status update
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# /checkout  — process shopping cart and create order
# -------------------------------------------------------------------
...
def checkout():
    ...
    try:
        ...
        for item in cart_items:
            ...
            # 1. Deduct Stock (Try to deduct from first warehouse that has enough)
            update_sql = """
            ...
            if cursor.rowcount == 0:
                # The update affected 0 rows meaning no warehouse has sufficient stock
...
def get_my_orders():
    ...
    # Get orders — use simple column select to avoid DATE_FORMAT escaping issues
    orders = query_db(
